...
public class MonkeyLevel2 extends BaseScreen {
	// ----MUSICA-------------------------
	...
	MonkeyLevel2(MonkeyGame game) {
		super(game);


		// ubicación del archivo
		game.set_json_data(
				"C:\\Users\\abdia_000\\Desktop\\versiones del juego\\MonkeyGame0.2\\core\\assets\\nivel2\\nivel2.json");
		...
		cant_enemigos2 = json.get_cant_enemigos2();
		cant_vidas = json.get_cant_vidas(); // va ha ver solo una vida en el
											// mapa que va a reaparecer dado un
											// tiempo
		cant_frutas = json.get_cant_frutas(); // cantidad de frutas que van a
												// aparecer en el mapa
		cant_objetos = json.get_cant_objetos(); // cantidad de frutas que van a
												// aparecer en el mapa
		...
		for (int i = 0; i < cant_vidas; i++) {
			...
			// cant_vidas de la "LISTA_VIDAS" (no del for, el del for indica los
			// objetos de "Vidas" a tomar en cuenta)
			// indica el numero de posiciones disponible que dispone una vida
			// para trasladarze
		...
		for (int i = 0; i < cant_enemigos; i++) {
			...
			lista_enemigos.get(i).__init__(json.get_matrix_enemigos()[i][0], json.get_matrix_enemigos()[i][1],
					json.get_matrix_enemigos()[i][2], json.get_matrix_enemigos()[i][3],
					json.get_matrix_enemigos()[i][4], json.get_matrix_enemigos()[i][5]);
		}
		...
		for (int i = 0; i < cant_objetos; i++) {
			...
			/*
			 * troncos{70, 644}, {133,0}, {326, 644}, {326, 580}, {581, 131},
			 * {645, 644}, {645, 580}, {710, 580}, {519,73}, {581,73},
			 * piedras{196, 261}, {196, 69}, {260, 261}, {260, 132}, {260, 68},
			 * {709, 133}, {709, 68}, {709, 4}, arboles{79, 72}
			 */
	...
	public void create() {
		...
		// ---MUSICA---------------------------------
		music_nivel = (Music) game.get_manager().get("nivel2/music_nivel2.mp3");
			...
			public void beginContact(Contact contact) {
				...
				if (areCollided(contact, "enemigos", "mono")) {
					...
					if (mono.cant_vidas > 1) {
						...
						mono.addAction(Actions.parallel(Actions.alpha(0), Actions.fadeIn(tiempo_resurreccion)));
					} else
						mono.vivo = false;
				}
	...
	public void show() {
		...
		// ---ACTORES-----------------------------------
		grama = new BaseActor();
		grama.setTexture((Texture) game.get_manager().get("nivel2/nivel_2.png"));
		...
		for (Enemy_2 e : lista_enemigos_2) {
			mainStage.addActor(e);
		}
        ...
        winTex.setTexture((Texture)game.get_manager().get("escenario/you-win2.png"));
		...
		this.nivelabel = new Label("NIVEL: 2-2", new LabelStyle(new BitmapFont(), Color.BLUE));
		...
		this.mono_siguiente_nivel = new Label("YOU HAVE WON, Press M to go to the menu",
				new LabelStyle(new BitmapFont(), Color.BLUE));
		...
		this.mono_muerto = new Label(" Press S to start again, M for main menu ",
				new LabelStyle(new BitmapFont(), Color.BLUE));
	...
	// para eleminar un objeto de el box2d con los pjs hay que eliminarlos
	// primero de box
	// si no da error
	...
	public void update(float dt) {
		...
		if (mono.vivo && !win) {
			...
			// aqui mueve al mono si chocÃ³ con un enimigo
			if (mono.golpeado) {
				mono.tiempo_resurreccion += dt;
				// mono.cuerpo.setTransform(0+0.5f, 0+0.5f, 0);
				mono.cuerpo.setTransform((json.get_mono_xy()[0] + mono.getOriginX()) / Constants.pixelametro,
						(json.get_mono_xy()[1] + mono.getOriginX()) / Constants.pixelametro, 0);
			...
			// process input
			if (!mono.golpeado) {
				if (Gdx.input.isKeyPressed(Keys.RIGHT)) { // Esto es la
															// transformacion de
															// pixeles a metros
															// |||| esto es la
															// velovidad en
			...
			// cam.position.set(mono.cuerpo.getPosition().x,
			// mono.cuerpo.getPosition().y, 0);
			// lÃ­mites de la camara
	...
	private void inic_acciones() {
		...
		fadeInColorCycleForever = Actions.sequence(Actions.alpha(0),
				// Actions.show(),
				Actions.fadeIn(2), Actions.forever(Actions.sequence(Actions.color(new Color(0.5f, 0.5f, 0.5f, 1), 1),
