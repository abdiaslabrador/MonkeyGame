...
public class Vidas extends BaseActor{
    ...
    public void __init__(int m[][], int tiempo){
        ...
        while (!ocupado){
            num_posicion=MathUtils.random(0, 1);
            if (m[num_posicion][2]==0){ //escoge la casilla que est� desocupado
    ...
    public void cambiar(int m[][]){
           ...
           while (!ocupado){ //mientras no esté ocupado busca un puesto y luego lo coloca ocupado
               num_posicion=MathUtils.random(0,1); //elige unas de las casillas de la matriz y luego comprueba si no esta ocupada con el numero 1
